...
abi vault {
    ...
    // // # Arguments
    // //
    // // * `singleSwap` - Struct that contains the information of poolId, SwapKind, assetIn, assetOut, amount, userData.
    // // * `funds` - Struct of funds mangament that contains the sender, fromInternalBalance, recipient, toInternalBalance.
    // // * `limits` - Array specifies the minimum or maximum amount of each token the vault is allowed to transfer.
    // // * `deadline` - deadline of transaction
    // //
    ...
    // # Arguments
    //
    // * `poolId` - The id of the token's pool
    // * `tokens` - Array of the contract ids of the tokens
    // * `assetManagers` - Array of the asset managers of the tokens
    #[storage(read, write)]
    fn register_tokens(
        poolId: b256,
    ...
    // * `request` - struct that contains the info of assets, maxAmountsIn, userData, fromInternalBalance
    ...
    // * `sender` - Address of the sender
    // * `recipient` - Address of the recipient
    // * `request` - struct that contains the info of assets, minAmountsOut, userData, toInternalBalance
    #[storage(read, write)]fn exit_pool(poolId: b256, specialization: PoolSpecialization, sender: Address, recipient: Address, request: ExitPoolRequest);
    ...
    // // * `userData` - The of the userData in
    // #[storage(read, write)]
    // fn flash_loan(
    ...
    // // # Arguments
    // //
    // // * `newAuthorizer` - The of the authorizer
    // #[storage(read, write)]
...
abi ExternalInterface {
    ...
    // todo this workaround for the vectors
    #[storage(read, write)]fn on_join_pool(poolId: b256, sender: ContractId, recipient: ContractId, balances: [u64; 2], lastChangeBlock: u64, protocolSwapFeePercentage: u64, userData: UserData) -> ([u64; 2], [u64; 2]);
    // #[storage(read, write)]
    // fn on_join_pool(
    //     poolId: b256,
    //     sender: ContractId,
    ...
    // * The array of fee of the due protocol fee amounts
    #[storage(read, write)]
    fn on_exit_pool(poolId: b256, sender: ContractId, recipient: ContractId, balances: [u64; 2], lastChangeBlock: u64, protocolSwapFeePercentage: u64, userData: UserData) -> ([u64; 2], [u64; 2]);
    // fn on_exit_pool(
    //     poolId: b256,
    //     sender: ContractId,
    //     recipient: ContractId,
