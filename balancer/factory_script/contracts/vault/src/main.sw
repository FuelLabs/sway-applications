...
storage {
    two_token_pool_tokens: StorageMap<b256,
    TwoTokenPoolTokens> = StorageMap {
    },
    // first b256 value is the pool id sencond b256 value is balances
    balances: StorageMap<(b256,
    b256), TwoTokenPoolBalances> = StorageMap {
    },
    internal_token_balance: StorageMap<(Address,
    ContractId), u64> = StorageMap {
    },
    minimal_swap_info_pools_balances: StorageMap<(b256,
    ContractId), b256> = StorageMap {
    },
    minimal_swap_info_pools_tokens: StorageMap<b256,
    Vec<ContractId>> = StorageMap {
    },
    general_pools_balances: StorageMap<b256,
    IERC20ToBytes32Map> = StorageMap {
    },
    entries: StorageMap<(b256,
    u64), IERC20ToBytes32MapEntry> = StorageMap {
    },
    indexes: StorageMap<(b256,
    ContractId), u64> = StorageMap {
    },
    pool_asset_managers: StorageMap<(b256,
    ContractId), Address> = StorageMap {
    },
    swap_fee_percentage: u64 = 1000000,
    flash_loan_fee_percentage: u64 = 1000000,
    is_pool_registered: StorageMap<b256,
    bool>= StorageMap {
    },
    next_pool_nonce: u64 = 0,
    approved_relayers: StorageMap<(Address,
    Address), bool> = StorageMap {
    },
...
impl vault for Contract {
    ...
    #[storage(read, write)]fn register_tokens(poolId: b256, tokens: Vec<ContractId>, assetManagers: Vec<Address>) {
        ...
        log(TokensRegistered {
            pool_id: poolId, tokens: tokens, asset_managers: assetManagers, 
        }
        );
    }
    ...
    #[storage(read, write)]fn deregister_tokens(poolId: b256, tokens: Vec<ContractId>) {
        ...
        let mut count = 0;
        while count < tokens.len() {
            storage.pool_asset_managers.insert((poolId, tokens.get(count).unwrap()), ~Address::from(ZERO_B256));
            count = count + 1;
        }
        log(TokensDeregistered {
            poolId: poolId, tokens: tokens, 
        }
        );
    }
    ...
    #[storage(read, write)]fn join_pool(poolId: b256, sender: Address, recipient: Address, request: JoinPoolRequest) {
        ...
        join_or_exit(PoolBalanceChangeKind::JOIN, poolId, sender, recipient, join_to_pool_balance_change(request));
    }

    #[storage(read, write)]fn exit_pool(poolId: b256, sender: Address, recipient: Address, request: ExitPoolRequest) {
        // This fn doesn't have the nonReentrant modifier: it is applied to `join_or_exit` instead.
        join_or_exit(PoolBalanceChangeKind::EXIT, poolId, sender, recipient, exit_to_pool_balance_change(request));
    }
    ...
    #[storage(read, write)]fn flash_loan(recipient: ContractId, tokens: Vec<ContractId>, amounts: Vec<u64>, userData: Vec<b256>, ) {
        ...
        while count < tokens.len() {
            ...
            log(FlashLoan {
                recipient: recipient, token: token, amount: amounts.get(count).unwrap(), receivedFeeAmount: receivedFeeAmount, 
            }
            );
    ...
    #[storage(read, write)]fn manage_pool_balance(ops: Vec<PoolBalanceOp>) {
        ...
        while count < ops.len() {
            ...
            log(PoolBalanceManaged {
                pool_id: pool_id, sender: sender, token: token, cash_delta: cash_delta, managed_delta: managed_delta_value, 
            }
            );
        }
    }
    ...
    #[storage(read, write)]fn set_relayer_approval(sender: Address, relayer: Address, approved: bool) {
        is_reentrant();
        storage.approved_relayers.insert((sender, relayer), approved);
...
#[storage(read, write)]fn deregister_two_token_pool_tokens(pool_id: b256, token_x: ContractId, token_y: ContractId) {
...
#[storage(read, write)]fn deregister_minimal_swap_info_pool_tokens(pool_id: b256, tokens: Vec<ContractId>) {
    let mut count = 0;
...
// helping function
#[storage(read, write)]fn vec_remove_if_contains(pool_id: b256, delete: ContractId) -> bool {
...
#[storage(write, read)]fn deregister_general_pool_tokens(pool_id: b256, tokens: Vec<ContractId>) {
...
#[storage(read)]fn get_general_pool_balance_storage_map(pool_id: b256, token: ContractId) -> b256 {
    return get_value(pool_id, token);
}

#[storage(read, write)]fn set(pool_id: b256, key: ContractId, value: b256) -> bool {
    ...
    if keyIndex == 0 {
        ...
        let tmp = IERC20ToBytes32MapEntry {
            key: storage.entries.get((pool_id,
            keyIndex - 1)).key, value: value,
        };
...
#[storage(read)]fn get_value(pool_id: b256, key: ContractId) -> b256 {
    let index = storage.indexes.get((pool_id, key));
    require(index > 0, Error::OUT_OF_BOUNDS);
    return unchecked_value_at(pool_id, index - 1);
}

#[storage(read)]fn unchecked_value_at(pool_id: b256, index: u64) -> b256 {
    return storage.entries.get((pool_id, index)).value;
}

#[storage(read, write)]fn register_two_token_pool_tokens(pool_id: b256, token_x: ContractId, token_y: ContractId) {
...
#[storage(read, write)]fn register_minimal_swap_info_pool_tokens(pool_id: b256, tokens: Vec<ContractId>) {
...
#[storage(write, read)]fn register_general_pool_tokens(pool_id: b256, tokens: Vec<ContractId>) {
    let pool_balances = storage.general_pools_balances.get(pool_id);
    let mut i = 0;
    while(i < tokens.len()) {
...
//         if storage.roles.get(role).get(count).unwrap() == account {
//             return true;
//         }
//         count = count + 1;
//     }
...
#[storage(write, read)]fn swap_with_pool(swaps: Vec<BatchSwapStep>, assets: Vec<ContractId>, funds: FundManagement, kind: SwapKind, ) -> Vec<u64> {
    ...
    while count < swaps.len() {
        ...
        // Initializing each struct field one-by-one uses less gas than setting all at once
        let poolRequest = SwapRequest {
            poolId: batchSwapStep.poolId, kind: kind, tokenIn: tokenIn, tokenOut: tokenOut, amount: batchSwapStep.amount, userData: batchSwapStep.userData, from: funds.sender, to: funds.recipient, // todo The lastChangeBlock field is left uninitialized.
            lastChangeBlock: 0, 
        };
...
#[storage(write, read)]fn swap_with_pool_hook(request: SwapRequest) -> (u64, u64, u64) {
...
#[storage(read)]fn process_two_token_pool_swap_request(request: SwapRequest, pool: ContractId) -> u64 {
    ...
    let mut poolBalances = storage.balances.get((request.poolId, pair_hash));
...
// Pools do this.
fn call_minimal_swap_info_pool_on_swap_hook(request: SwapRequest, pool: ContractId, tokenInBalance: b256, tokenOutBalance: b256) -> (b256, b256, u64) {
...
#[storage(read, write)]fn set_two_token_pool_cash_balances(pool_id: b256, token_a: ContractId, balance_a: b256, token_b: ContractId, balance_b: b256) {
...
#[storage(read, write)]fn set_minimal_swap_info_pool_balances(pool_id: b256, tokens: Vec<ContractId>, balances: Vec<b256>) {
    let mut count = 0;
...
#[storage(write, read)]fn set_general_pool_balances(pool_id: b256, balances: Vec<b256>) {
    let mut count = 0;
...
#[storage(read)]fn is_minimal_swap_info_pool_token_registered(pool_id: b256, token: ContractId) -> bool {
    let pool_tokens = storage.minimal_swap_info_pools_tokens.get(pool_id);
    return vec_contains(pool_tokens, token);
}

#[storage(read)]fn is_general_pool_token_registered(pool_id: b256, token: ContractId) -> bool {
    let pool_balances = storage.general_pools_balances.get(pool_id);
    return storage.indexes.get((pool_id, token)) != 0;
}

#[storage(read, write)]fn perform_pool_management_operation(kind: PoolBalanceOpKind, poolId: b256, token: ContractId, amount: u64) -> (u64, u64) {
    let specialization: PoolSpecialization = get_pool_specialization(poolId);
...
#[storage(read, write)]fn withdraw_pool_balance(poolId: b256, specialization: PoolSpecialization, token: ContractId, amount: u64) -> (u64, u64) {
    if let PoolSpecialization::TWO_TOKEN = specialization {
...
#[storage(read, write)]fn update_two_token_pool_shared_balance_cash_to_managed(pool_id: b256, token: ContractId, amount: u64) -> u64 {
    ...
    let balance = TwoTokenPoolBalances {
        shared_cash: to_shared_cash(balance_a,
        balance_b), shared_managed: to_shared_managed(balance_a,
        balance_b), 
    };
...
#[storage(read, write)]fn update_minimal_swap_info_pool_balance_cash_to_managed(pool_id: b256, token: ContractId, amount: u64) -> u64 {
...
#[storage(write, read)]fn update_general_pool_balance_cash_to_managed(pool_id: b256, token: ContractId, amount: u64) -> u64 {
    let pool_balances = storage.general_pools_balances.get(pool_id);
    let current_balance = get_general_pool_balance_storage_map(pool_id, token);
...
#[storage(read)]fn get_minimal_swap_info_pool_balance(pool_id: b256, token: ContractId) -> b256 {
    let balance = storage.minimal_swap_info_pools_balances.get((pool_id, token));
...
#[storage(read, write)]fn deposit_pool_balance(poolId: b256, specialization: PoolSpecialization, token: ContractId, amount: u64) -> (u64, u64) {
    if let PoolSpecialization::TWO_TOKEN = specialization {
...
// todo returns signed int in solidity. no support of signed int in sway
#[storage(read, write)]fn two_token_pool_managed_to_cash(pool_id: b256, token: ContractId, amount: u64) -> u64 {
    ...
    storage.balances.insert((pool_id, bal_hash), balance);

    return delta;
}

// returns signed int, but we dont have in sway
#[storage(read, write)]fn minimal_swap_info_pool_managed_to_cash(pool_id: b256, token: ContractId, amount: u64) -> u64 {
    let current_balance = get_minimal_swap_info_pool_balance(pool_id, token);
...
#[storage(write, read)]fn general_pool_managed_to_cash(pool_id: b256, token: ContractId, amount: u64) -> u64 {
    let pool_balances = storage.general_pools_balances.get(pool_id);
    let current_balance = get_general_pool_balance_storage_map(pool_id, token);
...
#[storage(read, write)]fn update_managed_balance(poolId: b256, specialization: PoolSpecialization, token: ContractId, amount: u64) -> (u64, u64) {
...
#[storage(read, write)]fn set_two_token_pool_managed_balance(pool_id: b256, token: ContractId, amount: u64) -> u64 {
    let(mut token_a, mut balance_a, _, mut balance_b) = get_two_token_pool_balances(pool_id);
    let mut delta: u64 = 0;
...
// returns signed int, but we dont have in sway
#[storage(read, write)]fn set_minimal_swap_info_pool_managed_balance(pool_id: b256, token: ContractId, amount: u64) -> u64 {
    let current_balance = get_minimal_swap_info_pool_balance(pool_id, token);
...
#[storage(write, read)]fn set_general_pool_managed_balance(pool_id: b256, token: ContractId, amount: u64) -> u64 {
    let pool_balances = storage.general_pools_balances.get(pool_id);
    let current_balance = get_general_pool_balance_storage_map(pool_id, token);
...
#[storage(read)]fn get_two_token_pool_shared_balances(pool_id: b256, token_x: ContractId, token_y: ContractId) -> (b256, b256) {
    let(token_a, token_b) = sort_two_tokens(token_x, token_y);
    let pair_hash = get_two_token_pair_hash(token_a, token_b);

    let shared_cash = storage.balances.get((pool_id, pair_hash)).shared_cash;
    let shared_managed = storage.balances.get((pool_id, pair_hash)).shared_managed;
...
#[storage(write, read)]fn process_minimal_swap_info_pool_swap_request(request: SwapRequest, pool: ContractId) -> u64 {
    ...
    // Perform the swap request and compute the new balances for 'token in' and 'token out' after the swap
    let(tokenInBalance, tokenOutBalance, amountCalculated) = call_minimal_swap_info_pool_on_swap_hook(request, pool, tokenInBalance, tokenOutBalance);
    storage.minimal_swap_info_pools_balances.insert((request.poolId, request.tokenIn), tokenInBalance);
    storage.minimal_swap_info_pools_balances.insert((request.poolId, request.tokenOut), tokenOutBalance);

    return amountCalculated;
}

#[storage(read, write)]fn process_general_pool_swap_request(request: SwapRequest, pool: ContractId) -> u64 {
    let mut request = request;
    ...
    let mut indexIn = storage.indexes.get((request.poolId, request.tokenIn));
    let mut indexOut = storage.indexes.get((request.poolId, request.tokenOut));
...
#[storage(read, write)]fn unchecked_set_at(pool_id: b256, index: u64, value: b256) {
    let mut entry = storage.entries.get((pool_id, index));
    entry.value = value;
    storage.entries.insert((pool_id, index), entry);
}

#[storage(read)]fn is_two_token_pool_token_registered(pool_id: b256, token: ContractId) -> bool {
...
#[storage(read, write)]fn join_or_exit(kind: PoolBalanceChangeKind, poolId: b256, sender: Address, recipient: Address, change: PoolBalanceChange) {
    ...
    let balances = validate_tokens_and_get_balances(poolId, tokens);
    ...
    log(PoolBalanceChanged {
        pool_id: poolId, sender: sender, tokens: tokens, balances: balances_log, paid_protocol_swap_fee_amounts: paidProtocolSwapFeeAmounts
    }
    );
}
...
#[storage(read)]fn validate_tokens_and_get_balances(poolId: b256, expectedTokens: Vec<ContractId>) -> Vec<b256> {
...
#[storage(read, write)]fn call_pool_balance_change(kind: PoolBalanceChangeKind, poolId: b256, sender: Address, recipient: Address, change: PoolBalanceChange, balances: Vec<b256>) -> (Vec<b256>, Vec<u64>, Vec<u64>) {
    let(totalBalances, lastChangeBlock) = totals_and_last_change_block(balances);

    let pool = get_pool_address(poolId);
    let get_protocol_swap_fee_percentage = MAX_PROTOCOL_SWAP_FEE_PERCENTAGE;
    // todo basePool function
    // if let PoolBalanceChangeKind::JOIN = kind{
...
// swap fees.
#[storage(read, write)]fn process_join_pool_transfers(sender: Address, change: PoolBalanceChange, balances: Vec<b256>, amountsIn: Vec<u64>, dueProtocolFeeAmounts: Vec<u64>) -> Vec<b256> {
    // We need to track how much of the received ETH was used and wrapped into WETH to return any excess.
    let mut wrappedEth = 0;
...
#[storage(read, write)]fn process_exit_pool_transfers(recipient: Address, change: PoolBalanceChange, balances: Vec<b256>, amountsOut: Vec<u64>, dueProtocolFeeAmounts: Vec<u64>) -> Vec<b256> {
    let mut finalBalances = ~Vec::new();
    let mut count = 0;
    while count < change.assets.len() {
...
#[storage(read, write)]fn deposit_to_internal_balance(asset: ContractId, sender: Address, recipient: Address, amount: u64) {
    increase_internal_balance(recipient, translate_to_ierc20(asset), amount);
    receive_asset(asset, amount, sender, false);
}

#[storage(read, write)]fn transfer_internal_balance(token: ContractId, sender: Address, recipient: Address, amount: u64) {
...
#[storage(read, write)]fn transfer_to_external_balance(token: ContractId, sender: Address, recipient: Address, amount: u64) {
    if (amount > 0) {
        transfer_to_output(amount, token, recipient);

        log(ExternalBalanceTransfer {
            amount: amount, recipient: recipient, sender: sender, token: token, 
        }
        );
    }
}

#[storage(read, write)]fn withdraw_from_internal_balance(asset: ContractId, sender: Address, recipient: Address, amount: u64) {
    // A partial decrease of Internal Balance is disallowed: `sender` must have the full `amount`.
...
#[storage(read, write)]fn increase_internal_balance(account: Address, token: ContractId, amount: u64) {
...
#[storage(read, write)]fn receive_asset(asset: ContractId, amount: u64, sender: Address, fromInternalBalance: bool) {
...
#[storage(read, write)]fn send_asset(asset: ContractId, amount: u64, recipient: Address, toInternalBalance: bool) {
    if (amount == 0) {
        return;
    }
...
// instead.
#[storage(read, write)]fn decrease_internal_balance(account: Address, token: ContractId, amount: u64, allowPartial: bool) -> u64 {
    let currentBalance: u64 = get_internal_balance(account, token);
...
// Todo When signed Integers are added
#[storage(read, write)]fn set_internal_balance(account: Address, token: ContractId, newBalance: u64, delta: u64, ) {
    storage.internal_token_balance.insert((account, token), newBalance);
    log(InternalBalanceChanged {
        account: account, delta: delta, token: token, 
    });
}

#[storage(read, write)]fn get_internal_balance(account: Address, token: ContractId) -> u64 {
    return storage.internal_token_balance.get((account, token));
}

// PoolTokens
#[storage(read)] // Returns all of `poolId`'s registered tokens, along with their raw balances.
fn private_get_pool_tokens(poolId: b256) -> (Vec<ContractId>, Vec<b256>) {
    let specialization: PoolSpecialization = get_pool_specialization(poolId);
    if let PoolSpecialization::TWO_TOKEN = specialization {
        return get_two_token_pool_tokens(poolId);
    } else if let PoolSpecialization::MINIMAL_SWAP_INFO = specialization {
        return get_minimal_swap_info_pool_tokens(poolId);
    } else {
        // PoolSpecialization::GENERAL
        return get_general_pool_tokens(poolId);
    }
}

#[storage(read)]fn get_two_token_pool_tokens(pool_id: b256) -> (Vec<ContractId>, Vec<b256>) {
...
#[storage(read)]fn get_minimal_swap_info_pool_tokens(pool_id: b256) -> (Vec<ContractId>, Vec<b256>) {
    let pool_tokens = storage.minimal_swap_info_pools_tokens.get(pool_id);
    let mut tokens = ~Vec::new();
    let mut balances = ~Vec::new();
...
#[storage(read)]fn get_general_pool_tokens(pool_id: b256) -> (Vec<ContractId>, Vec<b256>) {
    let pool_balances = storage.general_pools_balances.get(pool_id);
    let mut tokens = ~Vec::new();
    let mut balances = ~Vec::new();
...
#[storage(read)]fn unchecked_at(pool_id: b256, index: u64) -> (ContractId, b256) {
    let entry = storage.entries.get((pool_id, index));
    return(entry.key, entry.value);
}

#[storage(read)]fn get_two_token_pool_balances(pool_id: b256) -> (ContractId, b256, ContractId, b256) {
    let pool_tokens = storage.two_token_pool_tokens.get(pool_id);
