...
impl WeightedPool for Contract {
    ...
    #[storage(read, write)]fn on_join_pool(poolId: b256, sender: ContractId, recipient: ContractId, balances: Vec<u64>, lastChangeBlock: u64, protocolSwapFeePercentage: u64, userData: Vec<b256>) -> (Vec<u64>, Vec<u64>) {
        ...
        if TOTAL_SUPPLY == 0 {
            ...
            (amountsIn, ~Vec::with_capacity(balances.len()))
        }
    }

    // Vault hook for removing liquidity from a pool.
    // This fn can only be called from the Vault, from `exitPool`.
    #[storage(read, write)]fn on_exit_pool(poolId: b256, sender: ContractId, recipient: ContractId, balances: Vec<u64>, lastChangeBlock: u64, protocolSwapFeePercentage: u64, userData: Vec<b256>) -> (Vec<u64>, Vec<u64>) {
        let scalingFactors = scaling_factors();
        let balances = upscale_array(balances, scalingFactors);
    ...
    #[storage(read, write)]fn query_join(poolId: b256, sender: ContractId, recipient: ContractId, balances: Vec<u64>, lastChangeBlock: u64, protocolSwapFeePercentage: u64, userData: Vec<b256>) -> (u64, Vec<u64>) {
    ...
    // Like `IVault.queryBatchSwap`, this fn is not view due to internal implementation details: the caller must
    // explicitly use eth_call instead of eth_sendTransaction.
    #[storage(read, write)]fn query_exit(poolId: b256, sender: ContractId, recipient: ContractId, balances: Vec<u64>, lastChangeBlock: u64, protocolSwapFeePercentage: u64, userData: Vec<b256>) -> (u64, Vec<u64>) {
        require(balances.len() == TOTAL_TOKENS, Error::INPUT_LENGTH_MISMATCH);
...
// Exit
#[storage(read, write)]fn on_exit_pool_private(sender: ContractId, balances: Vec<u64>, protocolSwapFeePercentage: u64, scalingFactors: Vec<u64>, userData: Vec<b256>) -> (u64, Vec<u64>) {
...
#[storage(read)]fn do_exit(balances: Vec<u64>, normalizedWeights: Vec<u64>, scalingFactors: Vec<u64>, userData: Vec<b256>) -> (u64, Vec<u64>) {
    // let kind = userData.exitKind();

    // if (kind == WeightedPoolUserData.ExitKind.EXACT_BPT_IN_FOR_ONE_TOKEN_OUT) {
    //     return exit_exact_bptin_for_token_out(balances, normalizedWeights, userData);
    // } else if (kind == WeightedPoolUserData.ExitKind.EXACT_BPT_IN_FOR_TOKENS_OUT) {
    //     return exit_exact_bptin_for_tokens_out(balances, userData);
    // } else if (kind == WeightedPoolUserData.ExitKind.BPT_IN_FOR_EXACT_TOKENS_OUT) {
    //     return exit_bptin_for_exact_tokens_out(balances, normalizedWeights, scalingFactors, userData);
    // } else {
...
// Join
#[storage(read, write)]fn on_join_pool_private(sender: ContractId, balances: Vec<u64>, protocolSwapFeePercentage: u64, scalingFactors: Vec<u64>, userData: Vec<b256>) -> (u64, Vec<u64>) {
...
#[storage(read)]fn join_exact_tokens_in_for_bptout(balances: Vec<u64>, normalizedWeights: Vec<u64>, scalingFactors: Vec<u64>, userData: Vec<b256>) -> (u64, Vec<u64>) {
    // todo abi.decode
    // let(amountsIn, minBPTAmountOut) = userData.exactTokensInForBptOut();
    let amountsIn: Vec<u64> = ~Vec::new();
    let minBPTAmountOut = 0;
    // ensure_input_length_match(balances.len(), balances.len());
    require(balances.len() == balances.len(), Error::INPUT_LENGTH_MISMATCH);
...
#[storage(read)]fn do_join(balances: Vec<u64>, normalizedWeights: Vec<u64>, scalingFactors: Vec<u64>, userData: Vec<b256>) -> (u64, Vec<u64>) {
    // // todo abi.decode
    // // let kind = userData.joinKind();

    // if kind == JoinKind::EXACT_TOKENS_IN_FOR_BPT_OUT {
    //     join_exact_tokens_in_for_bptout(balances, normalizedWeights, scalingFactors, userData)
    // } else if kind == JoinKind::TOKEN_IN_FOR_EXACT_BPT_OUT {
    //     join_token_in_for_exact_bptout(balances, normalizedWeights, userData)
    // } else if kind == JoinKind::ALL_TOKENS_IN_FOR_EXACT_BPT_OUT {
    //     join_all_tokens_in_for_exact_bptout(balances, userData)
    // } else {
...
#[storage(read)]fn join_token_in_for_exact_bptout(balances: Vec<u64>, normalizedWeights: Vec<u64>, userData: Vec<b256>) -> (u64, Vec<u64>) {
    // todo abi.decode
    let(bptAmountOut, tokenIndex) = (0, 0);
...
#[storage(write)]fn on_initialize_pool(scalingFactors: Vec<u64>, userData: Vec<b256> // userData: JoinKind,
) -> (u64, Vec<u64>) {
    ...
    // todo: abi.decode
    // let kind = userData.joinKind();
    // require(JoinKind::INIT = kind, Error::UNINITIALIZED);

    // todo: abi.decode
    // let amountsIn: Vec<u64> = userData.initial_amounts_in(userData);
    let amountsIn = ~Vec::new();
