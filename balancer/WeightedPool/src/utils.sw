...
pub fn compute_join_exact_tokens_in_invariant_ratio(
    balances: Vec<u64>,
    normalizedWeights: Vec<u64>,
    amountsIn: Vec<u64>,
...
pub fn calculate_invariant(normalizedWeights: Vec<u64>, balances: Vec<u64>) -> u64 {
    ...
    let mut count = 0;
    while count < balances.len() {
        invariant = invariant * ((balances.get(count).unwrap() * normalizedWeights.get(count).unwrap()));
        count = count + 1;
    }
...
pub fn calc_bpt_in_given_exact_tokens_out(
    ...
    let mut count = 0;
    while count < balances.len() {
        balanceRatiosWithoutFee.push(div_up((balances.get(count).unwrap() - amountsOut.get(count).unwrap()), balances.get(count).unwrap()));
        invariantRatioWithoutFees = mul_up((invariantRatioWithoutFees + balanceRatiosWithoutFee.get(count).unwrap()), normalizedWeights.get(count).unwrap());
        count = count + 1;
    }

    let invariantRatio = compute_exit_exact_tokens_out_invariant_ratio(balances, normalizedWeights, amountsOut, balanceRatiosWithoutFee, invariantRatioWithoutFees, swapFeePercentage);
...
fn compute_exit_exact_tokens_out_invariant_ratio(
    ...
    while count < balances.len() {
        ...
        if (invariantRatioWithoutFees > balanceRatiosWithoutFee.get(count).unwrap())
        {
            ...
            let taxableAmount = amountsOut.get(count).unwrap() - nonTaxableAmount;
            let taxableAmountPlusFees = div_up(taxableAmount, complement(swapFeePercentage));
        ...
        let balanceRatio = div_down((balances.get(count).unwrap() - amountOutWithFee), balances.get(count).unwrap());
...
pub fn calc_token_out_given_exact_bpt_in(
    ...
    let invariantRatio = div_up((bptTotalSupply - bptAmountIn), bptTotalSupply);
    require(invariantRatio >= MIN_INVARIANT_RATIO, Error::MIN_BPT_IN_FOR_TOKEN_OUT);
