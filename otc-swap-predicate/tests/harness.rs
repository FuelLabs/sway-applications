...
async fn valid_predicate_spend_with_swap() {
    utils::test_predicate_spend_with_parameters(ASK_AMOUNT, ASK_TOKEN, RECEIVER_ADDRESS).await;
}

#[tokio::test]
#[should_panic]
async fn incorrect_ask_amount() {
    utils::test_predicate_spend_with_parameters(41, ASK_TOKEN, RECEIVER_ADDRESS).await;
}
...
async fn incorrect_receiver_address() {
    utils::test_predicate_spend_with_parameters(ASK_AMOUNT, ASK_TOKEN, Address::new([2u8; 32]))
        .await;
}
