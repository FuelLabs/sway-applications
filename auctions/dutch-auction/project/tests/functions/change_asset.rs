mod passing {
    use crate::utils::{auction, change_asset, create_auction, get_contract_instance};
...
mod failing {
    use crate::utils::{cancel_auction, change_asset, create_auction, get_contract_instance};
    use fuels::{
    ...
    async fn cannot_update_asset_for_invalid_id() {
        let (instance, wallet) = get_contract_instance().await;
        ...
        // Change the asset
        change_asset(&instance, ContractId::from([1; 32]), 2).await;
    }

    #[tokio::test]
    #[should_panic(expected = "InvalidAuctionID")]
    async fn cannot_update_asset_for_zero_id() {
        let (instance, wallet) = get_contract_instance().await;
        ...
        // Change the asset
        change_asset(&instance, ContractId::from([1; 32]), 0).await;
    }

    #[tokio::test]
    #[should_panic(expected = "SenderNotAuthor")]
    async fn cannot_update_asset_if_not_author() {
        let (instance, wallet) = get_contract_instance().await;
        ...
        instance
            ...
            .unwrap()
            .methods()
            .change_asset(ContractId::from([1; 32]), 1)
            .call()
            .await
            .unwrap();
    }
    ...
    async fn cannot_update_asset_if_auction_ended() {
        let (instance, wallet) = get_contract_instance().await;
