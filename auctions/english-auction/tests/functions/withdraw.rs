...
mod success {
    ...
    #[tokio::test]
    async fn buyer_withdrawls_tokens() {
        let (_, seller, buyer1, _, _, sell_token_contract_id, _, buy_token_contract_id, _) = setup().await;
    ...
    #[tokio::test]
    async fn seller_withdrawls_tokens() {
        let (_, seller, buyer1, _, _, sell_token_contract_id, _, buy_token_contract_id, _) = setup().await;
        let (sell_amount, initial_price, reserve_price, duration) = defaults_token().await;
    ...
    #[tokio::test]
    async fn out_bid_withdrawls_tokens() {
        let (_, seller, buyer1, buyer2, _, sell_token_contract_id, _, buy_token_contract_id, _) = setup().await;
        let (sell_amount, initial_price, reserve_price, duration) = defaults_token().await;
        ...
        mint_and_send_to_address(sell_amount, &seller.asset, seller.wallet.address().into()).await;
        mint_and_send_to_address(reserve_price, &buyer1.asset, buyer1.wallet.address().into()).await;
        mint_and_send_to_address(reserve_price, &buyer2.asset, buyer2.wallet.address().into()).await;
    ...
    #[tokio::test]
    async fn buyer_withdrawls_expired_auction() {
