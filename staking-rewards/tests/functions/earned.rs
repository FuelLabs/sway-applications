...
async fn calculate_earned_tokens() {
    ...
    staking_contract.methods().reward_per_token().call().await.unwrap();
    ...
    let reward_per_token_timestamp = wallet.get_provider().unwrap().chain_info().await.unwrap().latest_block.header.time.unwrap().timestamp() as u64;
