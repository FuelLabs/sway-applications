mod success {
    ...
    async fn gets_weight() {
        ...
        let initial_weight = approval_weight(&deployer.contract, users.get(0).unwrap().address)
            .await
            .value;

        constructor(&deployer.contract, users.clone()).await;

        let final_weight = approval_weight(&deployer.contract, users.get(0).unwrap().address)
            .await
            .value;
