mod success {
    ...
    async fn gets_weight() {
        ...
        let initial_weight = approval_weight(&deployer.contract, users.get(0).unwrap().address.into()).await.value;

        constructor(&deployer.contract, users.clone()).await;

        let final_weight = approval_weight(&deployer.contract, users.get(0).unwrap().address.into()).await.value;

        assert_eq!(initial_weight, 0);
        assert_ne!(initial_weight, final_weight);
        assert_eq!(final_weight, users.get(0).unwrap().weight);
    }
}
