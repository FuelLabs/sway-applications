...
mod success {
    ...
    async fn mints_to_multiple_wallets() {
        ...
        assert_eq!(
            wallet2.wallet.get_asset_balance(&AssetId::new(*deployer.asset_id)).await.unwrap(),
            0
        );
        mint_to(15, &deployer.simple_token, identity2.clone()).await;
        assert_eq!(
            wallet2.wallet.get_asset_balance(&AssetId::new(*wallet2.asset_id)).await.unwrap(),
            15
        ); 
    }
...
mod revert {
    ...
    async fn panics_when_sender_not_minter() {
        ...
        mint_to(10, &false_minter.simple_token, false_minter_identity.clone()).await;
    }
