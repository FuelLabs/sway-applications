...
import { screen, renderWithRouter, fireEvent, waitFor } from "@escrow/test-utils";
import { App } from "../../App";
...
describe("Accept arbiter", () => {
    ...
    it("should be able to accept arbiter", async () => {
        ...
        await waitFor(async () => {
            const buyerPageBtn = await screen.findByLabelText(/Go to buyer page/);
            expect(buyerPageBtn).toBeInTheDocument();
            await user.click(buyerPageBtn);
        });

        // Switch to buyer wallet
        mockUseWallet(buyer);

        const acceptArbiterBtn = await screen.findByLabelText(/Accept arbiter button/);
        expect(acceptArbiterBtn).toBeInTheDocument();
        await user.click(acceptArbiterBtn);
    });
});
