...
const createEscrow = async (deadline: BigNumberish, page: Page) => {
  ...
  const createEscrowBtn = page.locator('[aria-label="Create escrow"]');
  expect(createEscrowBtn).toContainText('PlusIconCreate Escrow');
  await createEscrowBtn.click();

  const toast = page.locator('text="New escrow created."');
  await toast.waitFor();
};
...
const buyerDeposit = async (page: Page) => {
  ...
  const toast = page.locator('text="Deposit successful."');
  await toast.waitFor();
};
...
test.describe('e2e', () => {
  ...
  test('Seller can take payment after deadline', async ({ page }) => {
    ...
    const toast = page.locator('text="Took payment successfully."');
    await toast.waitFor();
  });
